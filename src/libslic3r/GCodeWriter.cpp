...
std::string GCodeWriter::set_pressure_advance(double pa, int extruder_id) const
{
    std::ostringstream gcode;
    ...
    if (false) { // todo: bbl printer
        ...
        if (this->config.gcode_flavor == gcfKlipper) {
            // PandaForge: Support multi-extruder setups for Klipper
            // Klipper extruder naming: "extruder" (first), "extruder1", "extruder2", etc.
            gcode << "SET_PRESSURE_ADVANCE";
            if (extruder_id >= 0 && this->multiple_extruders) {
                if (extruder_id == 0)
                    gcode << " EXTRUDER=extruder";
                else
                    gcode << " EXTRUDER=extruder" << extruder_id;
            }
            gcode << " ADVANCE=" << std::setprecision(4) << pa << " ; Override pressure advance value\n";
        }
