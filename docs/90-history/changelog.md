...
Historical project notes. Do not treat this file as the current branch source of truth without checking `INDEX.md`, `docs/01-start-here/current-state.md`, and the live source tree.

This file is archive material. Some path mentions below reflect earlier doc layouts.

## Recent Changes

### March 25, 2026 -- Documentation Surface Cleaned Up For Agents
- Moved dated investigations and status reports under `docs/90-history/reports/`
- Added `docs/01-start-here/current-state.md` as the branch-reality summary
- Rewrote the docs router so agents can find the right file without loading the full doc set
- Updated core docs to match the actual flat repo layout and current branch paths
- Marked missing historical paths such as `tools/profile_converter/` and `src/slic3r/GUI/Klipper/` as archival context rather than active guidance
...
- Archived historical docs to `docs/90-history/`
