...
//            config.set_key_value("max_layer_z", new ConfigOptionFloat(m_max_layer_z));
        ...
        if (!gcodegen.is_BBL_Printer() && gcodegen.config().enable_pressure_advance.get_at(new_filament_id)) {
            ...
            gcode += gcodegen.writer().set_pressure_advance(gcodegen.config().pressure_advance.get_at(new_filament_id), new_extruder_id);
        }
...
*/
    ...
    // PandaForge: Set pressure advance for initial extruder at start of print
    // This is needed because set_extruder() only applies PA during toolchanges,
    // but for the first extruder no toolchange happens
    if (!this->is_BBL_Printer() && m_config.enable_pressure_advance.get_at(initial_extruder_id)) {
        int extruder_id = get_extruder_id(initial_extruder_id);
        file.write(m_writer.set_pressure_advance(m_config.pressure_advance.get_at(initial_extruder_id), extruder_id));
    }
...
std::string GCode::set_extruder(unsigned int new_filament_id, double print_z, bool by_object)
{
    ...
    if (!m_writer.multiple_extruders) {
        ...
        if (!this->is_BBL_Printer() && m_config.enable_pressure_advance.get_at(new_filament_id))
            gcode += m_writer.set_pressure_advance(m_config.pressure_advance.get_at(new_filament_id), new_extruder_id);
