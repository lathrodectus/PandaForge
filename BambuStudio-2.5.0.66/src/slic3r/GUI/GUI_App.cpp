...
/*#ifdef WIN32
    ...
    try {
        ...
        flush_logs();
        wxString errmsg = wxString::Format(_L("pandaforge will terminate because of running out of memory."
        ...
        flush_logs();
        wxString errmsg = _L("pandaforge will terminate because of a localization error. "
        ...
        BOOST_LOG_TRIVIAL(error) << boost::format("Uncaught exception: %1%") % ex.what();
        flush_logs();
        wxLogError(format_wxstr(_L("pandaforge got an unhandled exception: %1%"), ex.what()));
        throw;
    }
//#endif
}
...
{
	//app config initializes early becasuse it is used in instance checking in pandaforge.cpp
...
static boost::optional<Semver> parse_semver_from_ini(std::string path)
{
    ...
    size_t start = body.find("pandaforge ");
	...
	if (m_app_conf_exists) {
        ...
        if (!error.empty()) {
            ...
            throw Slic3r::RuntimeError(
                _u8L("pandaforge configuration file read failed. Please manually backup and delete it, and then restart pandaforge software.") +
...
bool GUI_App::on_init_inner()
{
    ...
    BOOST_LOG_TRIVIAL(info) << boost::format("gui mode, Current pandaforge Version %1%")%SLIC3R_VERSION << ", BuildTime " << SLIC3R_BUILD_TIME;
...
//     Slic3r::debugf "wxWidgets version %s, Wx version %s\n", wxVERSION_STRING, wxVERSION;
        ...
        if (!msg.empty() && !ssl_accept) {
            RichMessageDialog
                dlg(nullptr,
                    ...
                    "pandaforge", wxICON_QUESTION | wxYES_NO);
    ...
    if (is_editor()) {
        ...
        Bind(EVT_ENTER_FORCE_UPGRADE, [this](const wxCommandEvent& evt) {
                ...
                DownloadDialog dialog(this->mainframe,
                    tips,
                    _L("The pandaforge needs an upgrade"),
                    false,
                    wxCENTER | wxICON_INFORMATION);
...
void GUI_App::on_http_error(wxCommandEvent &evt)
{
    ...
    if (code == HttpErrorVersionLimited) {
        if (!m_show_error_msgdlg) {
            MessageDialog msg_dlg(nullptr, _L("The Bambu Studio version is too old to enable cloud service. Please download the latest version from pandaforge repository."), "", wxAPPLY | wxOK);
        ...
        if (!m_show_error_msgdlg) {
            MessageDialog msg_dlg(nullptr, _L("Your software certificate has been revoked, please update pandaforge software."), "", wxAPPLY | wxOK);
            m_show_error_msgdlg = true;
            auto modal_result = msg_dlg.ShowModal();
            m_show_error_msgdlg = false;
            return;
...
bool GUI_App::process_network_msg(std::string dev_id, std::string msg)
{
    if (dev_id.empty()) {
        if (msg == "wait_info") {
            ...
            if (!m_show_error_msgdlg) {
                MessageDialog msg_dlg(nullptr, _L("Please try updating pandaforge and then try again."), "", wxAPPLY | wxOK);
                m_show_error_msgdlg = true;
                auto modal_result = msg_dlg.ShowModal();
                m_show_error_msgdlg = false;
            }
            ...
            if (!m_show_error_msgdlg) {
                MessageDialog msg_dlg(nullptr, _L("The certificate has expired. Please check the time settings or update pandaforge and try again."), "", wxAPPLY | wxOK);
                m_show_error_msgdlg = true;
                auto modal_result = msg_dlg.ShowModal();
                m_show_error_msgdlg = false;
            }
...
bool GUI_App::load_language(wxString language, bool initial)
{
    ...
    if (initial) {
        ...
        if (! language.empty())
        	BOOST_LOG_TRIVIAL(info) << boost::format("language provided by pandaforge.conf: %1%") % language;
        else {
	...
	if (language_info != nullptr && language_info->LayoutDirection == wxLayout_RightToLeft) {
    	BOOST_LOG_TRIVIAL(info) << boost::format("The following language code requires right to left layout, which is not supported by pandaforge: %1%") % language_info->CanonicalName.ToUTF8().data();
		language_info = nullptr;
	}
    ...
    if (! wxLocale::IsAvailable(language_info->Language)) {
    	...
    	wxString message = "Switching pandaforge to language " + language_info->CanonicalName + " failed.";
        ...
        wxMessageBox(message, "pandaforge - Switching language failed", wxOK | wxICON_ERROR);
        if (initial)
...
void GUI_App::OSXStoreOpenFiles(const wxArrayString &fileNames)
{
    ...
    if (fileNames.size() == num_gcodes) {
        // Opening PrusaSlicer by drag & dropping a G-Code onto pandaforge icon in Finder,
...
void GUI_App::associate_files(std::wstring extend)
{
    ...
    std::wstring prog_id = L" Bambu.Studio.1";
    std::wstring prog_desc = L"pandaforge";
